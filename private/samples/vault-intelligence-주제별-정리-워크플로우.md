...
#### 2-1. 주제별 문서 수집 실행
```bash
python -m src collect "[주제명]" --output [주제명]-topics.md
```

**예제**:
```bash
python -m src collect "TDD" --output tdd-topics.md
```
...
```bash
# 영문 확장 검색
python -m src search "[English Topic Name]" --top-k 15

# 관련 키워드 검색  
python -m src search "[관련 키워드]" --top-k 10
```

**예제**:
```bash
python -m src search "Test Driven Development" --top-k 15
python -m src search "unit test" --top-k 10
python -m src search "refactoring" --top-k 10
```
...
# 1. 기본 수집
echo "1️⃣ 기본 문서 수집..."
python -m src collect "$TOPIC" --output "$OUTPUT_DIR/${TOPIC}-topics.md"

# 2. 확장 검색
echo "2️⃣ 확장 검색 실행..."
python -m src search "$TOPIC" --top-k 15 > "$OUTPUT_DIR/${TOPIC}-search-results.txt"

# 3. 영문 검색 (필요시)
echo "3️⃣ 영문 검색 실행..."
python -m src search "$TOPIC english equivalent" --top-k 10 >> "$OUTPUT_DIR/${TOPIC}-search-results.txt"
...
```bash
# 해결책: 유사도 임계값 조정
python -m src search "[주제]" --threshold 0.1
```
